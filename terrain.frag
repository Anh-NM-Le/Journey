...
vec3 shading_terrain(vec3 pos) {
	...
	float h = pos.z + .8;
	h = clamp(h, 0.0, 1.0);
	float depth = clamp(1.0 - h, 0.0, 1.0);
	vec3 emissiveColor = mix(vec3(0,.1,.3), vec3(1.,1.,1.), h);

	return color * emissiveColor;

	// vec3 baseColor = shading_phong(lt[0], e, p, s, n).xyz;

	// //snow
	// float heightFactor = 1;
	// float snowLevel = 0.8;
	// float snowBlend = smoothstep(snowLevel - 0.1, snowLevel + 0.1, heightFactor);
	// vec3 snowColor = vec3(1.0, 1.0, 1.0);

	// //River
	// float riverProx = length(pos.xy - vec2(0.0, 0.0));
	// float riverWidth = 0.2;
	// float riverBlend = smoothstep(riverWidth, riverWidth + 0.1, riverProx);
	// vec3 riverColor = vec3(0.2, 0.4, 0.8);

	// vec3 groundColor = vec3(0.4, 0.3, 0.2);

	// vec3 blendedColor = mix(groundColor, riverColor, 1.0 - riverBlend);
	// blendedColor = mix(blendedColor, snowColor, snowBlend);

	// return baseColor * blendedColor;
}
